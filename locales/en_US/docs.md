...
Pepcrypt is a free [opensource] web app that provides secure file encryption in the browser.
...
---
It's easy to self host and deploy Pepcrypt, you can do that either with npm or docker
...
```bash
git clone https://github.com/sh-dv/Pepcrypt.git Pepcrypt
```
...
```bash
docker pull shdv/Pepcrypt:latest
```

2. run container

```bash
docker run -d -p 3991:80 shdv/Pepcrypt
```
...
Pepcrypt is also available as a Docker image. You can find it on [Docker Hub].
...
Files encrypted with Pepcrypt are identifiable by looking at the file signature that is used by the app to verify the content of a file, Such signatures are also known as magic numbers or Magic Bytes. These Bytes are authenticated and cannot be changed.
...
No, Pepcrypt never stores any of your data. It only runs locally in your browser.

<hr style="height: 1px">

### Is Pepcrypt free?

Yes, Pepcrypt is free and always will be. However, please consider [donating](https://github.com/sh-dv/Pepcrypt#donations) to support the project.

<hr style="height: 1px">

### Which file types are supported? Is there a file size limit?

Pepcrypt accepts all file types. There's no file size limit, meaning files of any size can be encrypted.
...
Pepcrypt uses authenticated encryption. The sender must provide their private key, a new shared key will be computed from both keys to encrypt the file. Recipient has to provide their private key when decrypting also. this way can verify that the encrypted file was not tampered with, and was sent from the real sender.
...
You can generate keys by visit the [key generate page](https://Pepcrypt/generate-keys), make sure to [store the keys safely](#best-practices).
...
Pepcrypt is an open-source application. You can help make it better by making commits on GitHub. The project is maintained in my free time. [Donations](https://github.com/sh-dv/Pepcrypt#donations) of any size are appreciated.
...
`stream.enqueue()` function adds the Pepcrypt signature(magic bytes), salt and header followed by the encrypted chunks.
...
[opensource]: https://github.com/sh-dv/Pepcrypt
...
[github]: https://github.com/sh-dv/Pepcrypt
[veracrypt]: https://veracrypt.fr
[cryptomator]: https://cryptomator.org
[kryptor]: https://github.com/samuel-lucas6/Kryptor
[gpg]: https://gnupg.org
[docker hub]: https://hub.docker.com/r/shdv/Pepcrypt
